...
import { type AppState, FormPosition } from '../types';
...
export class SwingAnalyzerViewModel {
  ...
  private handlePipelineUpdate(result: PipelineResult): void {
    ...
    if (currentTime - this.lastPipelineLogTime >= this.logThrottleMs) {
      ...
      console.log(
        '===== SwingAnalyzerViewModel: PIPELINE UPDATE RECEIVED ====='
      );
      console.log(
        'SwingAnalyzerViewModel: RepCount:',
        this.pipeline.getRepCount()
      );
      console.log('SwingAnalyzerViewModel: Has Skeleton:', !!result.skeleton);

      if (result.skeleton) {
        console.log(
          'SwingAnalyzerViewModel: Skeleton angle:',
          result.skeleton.getSpineAngle().toFixed(2)
        );
        console.log(
          'SwingAnalyzerViewModel: Keypoints:',
          result.skeleton.getKeypoints().length
        );
      }

      console.log(
        '===== SwingAnalyzerViewModel: PIPELINE UPDATE PROCESSED ====='
      );
    }
  ...
  private updateRepCounterDisplay(): void {
    if (this.repCounterElement) {
      this.repCounterElement.textContent = this.pipeline
        .getRepCount()
        .toString();
    }
  }
  ...
  private renderSkeleton(skeleton: Skeleton): void {
    ...
    if (skeleton) {
      ...
      if (currentTime - this.lastRenderLogTime >= this.logThrottleMs) {
        ...
        console.log(
          'SwingAnalyzerViewModel: Rendering skeleton with',
          skeleton.getKeypoints().length,
          'keypoints'
        );
        console.log(
          'SwingAnalyzerViewModel: Canvas dimensions:',
          this.canvasElement.width,
          'x',
          this.canvasElement.height
        );
        console.log(
          'SwingAnalyzerViewModel: Skeleton visibility:',
          skeleton.hasRequiredKeypoints()
        );
      }
      ...
      if (currentTime - this.lastRenderLogTime >= this.logThrottleMs) {
        console.log(
          '===== SwingAnalyzerViewModel: SKELETON RENDERING COMPLETE ====='
        );
      }
    } else {
