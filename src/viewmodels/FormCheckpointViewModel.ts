...
import { AppState, FormCheckpoint, FormPosition } from '../types';
import { Pipeline } from '../pipeline/Pipeline';
import { CheckpointEvent } from '../pipeline/PipelineInterfaces';
...
export class FormCheckpointViewModel {
  ...
  private fullscreenCanvas: HTMLCanvasElement = document.createElement('canvas');
  ...
  private checkpointsPerRep: Map<number, Map<FormPosition, FormCheckpoint>> = new Map();
  ...
  private subscribeToCheckpointEvents(): void {
    ...
    this.checkpointSubscription = this.pipeline.getCheckpointEvents().subscribe(
      ...
      (error: unknown) => console.error('Error in checkpoint subscription:', error)
    );
  }
  ...
  private handleCheckpointEvent(event: CheckpointEvent): void {
    ...
    if (event.checkpoint && event.position) {
      ...
      if (!this.checkpointsPerRep.has(repCount)) {
        this.checkpointsPerRep.set(repCount, new Map<FormPosition, FormCheckpoint>());
      }
  ...
  private navigateFullscreen(direction: 'prev' | 'next'): void {
    ...
    if (repMap && repMap.has(newPosition)) {
  ...
  updateDisplay(): void {
    ...
    for (const { pos, x, y } of positions) {
      ...
      this.displayCtx.fillText(
        checkpoint ? `${posName} (${Math.abs(checkpoint.spineAngle).toFixed(1)}°)` : posName,
        cellX + cellWidth / 2,
        cellY + 20
      );
      ...
      if (checkpoint) {
        ...
        this.displayCtx.fillRect(cellX + 10, cellY + 40, cellWidth - 20, cellHeight - 50);
  ...
  private clearDisplay(): void {
    ...
    this.displayCtx.fillRect(0, 0, this.displayGridCanvas.width, this.displayGridCanvas.height);
  }
