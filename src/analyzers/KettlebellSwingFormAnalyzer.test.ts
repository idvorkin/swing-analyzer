...
function createMockSkeleton(angles: {
  ...
  return {
    ...
    // New methods for dominant arm detection - return null to skip detection in tests
...
describe('KettlebellSwingFormAnalyzer', () => {
  ...
  describe('dominant arm detection', () => {
    /**
     * Creates a mock Skeleton with facing direction and wrist positions
     * for testing dominant arm detection.
     */
    function createMockSkeletonWithFacing(
      angles: { arm: number; spine: number; hip: number; knee: number; wristHeight: number },
      facing: 'left' | 'right' | null,
      wristPositions: { left: number | null; right: number | null }
    ): Skeleton {
      return {
        getArmToVerticalAngle: vi.fn().mockReturnValue(angles.arm),
        getSpineAngle: vi.fn().mockReturnValue(angles.spine),
        ...
        getKneeAngle: vi.fn().mockReturnValue(angles.knee),
        getWristHeight: vi.fn().mockReturnValue(angles.wristHeight),
        getFacingDirection: vi.fn().mockReturnValue(facing),
        getWristX: vi.fn().mockImplementation((side: 'left' | 'right') =>
          side === 'left' ? wristPositions.left : wristPositions.right
        ),
      } as unknown as Skeleton;
    }

    it('detects right arm as dominant when facing right and right wrist is forward', () => {
      const analyzer = new KettlebellSwingFormAnalyzer();

      // Simulate hinged frames where facing=right and rightWrist > leftWrist
      const hingedAngles = { arm: -10, spine: 50, hip: 120, knee: 160, wristHeight: -100 };

      // Process enough hinged frames to lock in dominant arm (needs 5 votes)
      for (let i = 0; i < 6; i++) {
        const skeleton = createMockSkeletonWithFacing(
          hingedAngles,
          'right',
          { left: 1000, right: 1100 } // right wrist further right = front
        );
        analyzer.processFrame(skeleton, i * 10);
      }

      // The getArmToVerticalAngle should have been called with 'right' preference
      // after locking in. We verify by checking the mock was called.
      const skeleton = createMockSkeletonWithFacing(
        hingedAngles,
        'right',
        { left: 1000, right: 1100 }
      );
      analyzer.processFrame(skeleton, 100);

      // Check that arm angle was requested (the mock returns the preset value)
      expect(skeleton.getArmToVerticalAngle).toHaveBeenCalled();
    });

    it('detects left arm as dominant when facing right but left wrist is forward', () => {
      const analyzer = new KettlebellSwingFormAnalyzer();
      ...
      for (let i = 0; i < 6; i++) {
        const skeleton = createMockSkeletonWithFacing(
          hingedAngles,
          'right',
          { left: 1200, right: 1100 } // left wrist further right = front when facing right
        );
        analyzer.processFrame(skeleton, i * 10);
      }

      expect(analyzer.getPhase()).toBeDefined(); // Should have processed without error
    });

    it('does not detect dominant arm when not hinged', () => {
      const analyzer = new KettlebellSwingFormAnalyzer();

      // Top position (spine < 30)
      const topAngles = { arm: 80, spine: 10, hip: 170, knee: 170, wristHeight: 50 };
      ...
      // Should not have locked in since spine < 30 (not hinged)
      // The analyzer should still work, just using fallback arm selection
      expect(analyzer.getPhase()).toBe('top');
    });

    it('does not change dominant arm after locked', () => {
      const analyzer = new KettlebellSwingFormAnalyzer();

      const hingedAngles = { arm: -10, spine: 50, hip: 120, knee: 160, wristHeight: -100 };

      // First, lock in RIGHT arm
      for (let i = 0; i < 6; i++) {
        const skeleton = createMockSkeletonWithFacing(
          hingedAngles,
          'right',
          { left: 1000, right: 1100 } // right is front
        );
        analyzer.processFrame(skeleton, i * 10);
      }

      // Now try to change to left (should be ignored)
      for (let i = 0; i < 10; i++) {
        const skeleton = createMockSkeletonWithFacing(
          hingedAngles,
          'right',
          { left: 1200, right: 1100 } // left is front now
        );
        analyzer.processFrame(skeleton, 100 + i * 10);
      }

      // The analyzer should still function correctly
      expect(analyzer.getPhase()).toBeDefined();
    });

    it('resets dominant arm detection on reset()', () => {
      const analyzer = new KettlebellSwingFormAnalyzer();
      ...
      // Should be able to detect a different arm now
      expect(analyzer.getPhase()).toBe('top');
      expect(analyzer.getRepCount()).toBe(0);
    });
  });
