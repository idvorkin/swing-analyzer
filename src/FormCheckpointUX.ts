import { FormCheckpointLogic } from './FormCheckpointLogic';
import {
  type FormCheckpoint,
  FormPosition,
  type PoseKeypoint,
...
export class FormCheckpointUX {
  ...
  private logic: FormCheckpointLogic;
  ...
  constructor(
    ...
    this.logic = new FormCheckpointLogic(videoElement, canvasElement);
  ...
  private navigateFullscreen(direction: 'prev' | 'next'): void {
    ...
    const reps = this.logic.getAllReps();
    ...
    if (direction === 'next') {
      ...
      if (currentPositionIndex === positions.length - 1) {
        ...
        if (this.currentFullscreenRep < reps.length - 1) {
    ...
    const rep = reps[this.currentFullscreenRep];
    if (!rep) return;
  ...
  private displayFullscreenCheckpoint(
    ...
    if (ctx) {
      ...
      ctx.fillStyle = 'white';
      ctx.fillText(
        `Rep #${repNumber} - ${this.logic.formatPositionName(checkpoint.position)} - ${checkpoint.spineAngle.toFixed(1)}°`,
        20,
        38
      );
  ...
  private handleGridClick(event: MouseEvent): void {
    const currentRep = this.logic.getCurrentRep();
    ...
    if (checkpoint) {
      ...
      const positionName = this.logic.formatPositionName(position);
  ...
  private showFullscreen(checkpoint: FormCheckpoint): void {
    if (!this.fullscreenOverlay || !this.fullscreenCanvas) return;
    ...
    // Find the rep index
    const allReps = this.logic.getAllReps();
    this.currentFullscreenRep = allReps.findIndex(
      (rep) => rep.repNumber === currentRep.repNumber
    );
    if (this.currentFullscreenRep === -1 && allReps.length > 0) {
      // If the current rep isn't in the saved reps (likely the active rep),
      // use the last saved rep
      this.currentFullscreenRep = allReps.length - 1;
    }

    // Display the checkpoint
    this.displayFullscreenCheckpoint(checkpoint, currentRep.repNumber);
  ...
  updateDisplay(): void {
    ...
    for (const { pos, x, y } of positions) {
      const checkpoint = currentRep.checkpoints.get(pos);
      if (checkpoint) {
        ...
        this.displayCanvasContext.fillText(
          `${this.logic.formatPositionName(pos)} (${checkpoint.spineAngle.toFixed(1)}°)`,
          x + 10,
          y + 25
        );
      } else {
        ...
        this.displayCanvasContext.font = 'bold 16px Arial';
        this.displayCanvasContext.fillText(
          `${this.logic.formatPositionName(pos)} - waiting...`,
          x + 10,
          y + 25
        );
      }
    ...
    this.displayCanvasContext.font = 'bold 16px Arial';
    this.displayCanvasContext.fillText(
      `Repetition #${currentRep.repNumber}`,
      15,
      gridHeight - 15
    );
  }
  ...
  private clearDisplay(): void {
    ...
    if (!this.logic.getCurrentRep()) {
      ...
      this.displayCanvasContext.lineWidth = 2;
      this.displayCanvasContext.strokeRect(
        width / 4,
        height / 2 - 40,
        width / 2,
        80
  ...
  // Public methods that delegate to the logic component
  ...
  processFrame(
    ...
    this.logic.processFrame(keypoints, spineAngle, repCount);
    this.updateDisplay();
  }
  ...
  navigateToRep(repIndex: number): boolean {
    const result = this.logic.navigateToRep(repIndex);
    if (result) {
      this.updateDisplay();
    }
    return result;
