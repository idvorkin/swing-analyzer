...
import { type RepData, SwingPositionName } from '../types';
import type { FormEvent, RepEvent, RepProcessor } from './PipelineInterfaces';
...
export class SwingRepProcessor implements RepProcessor {
  ...
  updateRepCount(checkpointEvent: FormEvent): Observable<RepEvent> {
    console.log(
      `SwingRepProcessor: Received checkpoint event for position ${checkpointEvent.position}`
    );

    if (!checkpointEvent.skeletonEvent.skeleton || !checkpointEvent.position) {
      console.warn(
        `SwingRepProcessor: Invalid checkpoint event - missing skeleton or position`
      );
    ...
    console.log(
      `SwingRepProcessor: Added position ${currentPosition}, detected positions now: ${Array.from(this.detectedPositions).join(', ')}`
    );
    ...
    if (
      ...
      console.log(
        `SwingRepProcessor: Detected transition from Release to Top, checking for rep completion`
      );
      // Check if we've seen all positions in the correct sequence
      if (this.hasCompletedFullCycle()) {
        console.log(
          `SwingRepProcessor: Full cycle detected, incrementing rep count`
        );
        ...
        console.log(
          `Rep ${this.repCount} detected - full swing cycle completed`,
          {
            top: {
              spineAngle: this.currentRep?.checkpoints.get(
                SwingPositionName.Top
              )?.spineAngle,
              armToSpineAngle: this.currentRep?.checkpoints.get(
                SwingPositionName.Top
              )?.armToSpineAngle,
            },
            connect: {
        ...
        console.log(
          `SwingRepProcessor: Full cycle not detected, positions detected: ${Array.from(this.detectedPositions).join(', ')}`
        );
      }
    }
    ...
    console.log(
      `SwingRepProcessor: Updated last position to ${this.lastPosition}`
    );
    ...
    if (checkpointEvent.checkpoint && this.currentRep) {
      ...
      console.log(
        `SwingRepProcessor: Stored checkpoint for position ${checkpointEvent.checkpoint.position} in rep ${this.currentRep.repNumber}`
      );
    }
    ...
    if (this.currentRep === null) {
      ...
      console.log(
        `SwingRepProcessor: Created new rep with number ${this.currentRep.repNumber}`
      );
      ...
      if (checkpointEvent.checkpoint) {
        ...
        console.log(
          `SwingRepProcessor: Added first checkpoint for position ${checkpointEvent.checkpoint.position}`
        );
      }
    }

    console.log(
      `SwingRepProcessor: Emitting RepEvent with repCount=${this.repCount}, repIncremented=${repIncremented}`
    );
    return of({
      repCount: this.repCount,
      checkpointEvent,
  ...
  calculateRepMetrics(): Record<string, any> {
    ...
    return {
      top: {
        spineAngle: this.currentRep?.checkpoints.get(SwingPositionName.Top)
          ?.spineAngle,
        armToSpineAngle: this.currentRep?.checkpoints.get(SwingPositionName.Top)
          ?.armToSpineAngle,
      },
      connect: {
        spineAngle: this.currentRep?.checkpoints.get(SwingPositionName.Connect)
          ?.spineAngle,
        armToSpineAngle: this.currentRep?.checkpoints.get(
          SwingPositionName.Connect
        )?.armToSpineAngle,
      ...
      bottom: {
        spineAngle: this.currentRep?.checkpoints.get(SwingPositionName.Bottom)
          ?.spineAngle,
        armToSpineAngle: this.currentRep?.checkpoints.get(
          SwingPositionName.Bottom
        )?.armToSpineAngle,
      ...
      release: {
        spineAngle: this.currentRep?.checkpoints.get(SwingPositionName.Release)
          ?.spineAngle,
        armToSpineAngle: this.currentRep?.checkpoints.get(
          SwingPositionName.Release
        )?.armToSpineAngle,
