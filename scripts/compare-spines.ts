...
for (const targetTime of bottomTimes) {
  const fixtureFrame = fixture.frames.find(
    (f: any) => Math.abs(f.videoTime - targetTime) < 0.1
  );
  const tfliteFrame = tflite.frames.find(
    (f: any) => Math.abs(f.videoTime - targetTime) < 0.1
  );
  ...
  console.log(
    targetTime.toFixed(1).padStart(6) +
      's | ' +
...
console.log(
  'Fixture max: ' +
    fixtureMaxSpine.toFixed(1) +
    '° at ' +
    fixtureMaxTime.toFixed(2) +
    's'
);
console.log(
  'TFLite max:  ' +
    tfliteMaxSpine.toFixed(1) +
    '° at ' +
    tfliteMaxTime.toFixed(2) +
    's'
);
