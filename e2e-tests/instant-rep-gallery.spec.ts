...
test.describe
  .serial('Instant Rep Gallery: Reps Appear During Extraction', () => {
    ...
    test('rep gallery shows thumbnails during extraction without pressing play', async ({
      ...
      page.on('console', (msg) => {
        ...
        if (
          ...
          text.includes('Form processor') ||
          text.includes('Rep processor') ||
          text.includes('checkpoint') ||
          text.includes('CYCLE') ||
          text.includes('Found') ||
          text.includes('Emitting') ||
          text.includes('position') ||
          text.includes('FormAnalyzer') ||
          text.includes('Filmstrip') ||
    ...
    test('rep gallery thumbnails appear during extraction', async ({
      ...
      console.log(
        `Rep gallery has ${thumbnailCount} thumbnails after extraction`
      );
    ...
    test('extraction with mock detector produces rep gallery with multiple reps', async ({
      ...
      test.info().annotations.push({
        type: 'retry',
        description: 'Flaky with parallel IndexedDB access',
      });
      ...
      console.log(
        `Detected ${info.thumbnails} thumbnails, rep info: "${info.repInfo}"`
      );
...
test.describe
  .serial('Playback Mode: No Duplicate Rep Counting', () => {
    test.beforeEach(async ({ page }) => {
...
test.describe
  .serial('Rep Gallery Frame Capture During Extraction', () => {
    test.beforeEach(async ({ page }) => {
      // Intercept GitHub video URL and serve short local video for faster tests
      await useShortTestVideo(page);
      await page.goto('/');
 ...
 */
test.describe
  .serial('Skeleton Rendering Performance', () => {
    test.beforeEach(async ({ page }) => {
      await useShortTestVideo(page);
      await page.goto('/');
     ...
     */
    test('skeleton renders per-frame during playback', async ({ page }) => {
      // Fast extraction
      await setupMockPoseDetector(page, 'swing-sample-4reps', 0);
      ...
      const renderCount = await page.evaluate(() => {
        return (
          (
            window as unknown as { __renderCount?: () => number }
          ).__renderCount?.() || 0
        );
      });
    ...
    test('canvas content changes during video playback', async ({ page }) => {
      ...
      await page.evaluate(() => {
        ...
        if (canvas) {
          ...
          (
            window as unknown as { __canvasChangeCount: () => number }
          ).__canvasChangeCount = () => changeCount;
        }
      });
