...
test.describe('User Journey: Load and Analyze Sample Video', () => {
  ...
  // Skip: Filmstrip tests require ML model for batch analysis
  // These will work with mock pose detector infrastructure from PR #46
  test.describe
    .skip('Filmstrip Frame Capture', () => {
      test('filmstrip shows captured images WITHOUT playing video (batch analysis)', async ({
        page,
      }) => {
      ...
      test('filmstrip shows captured images after loading video with cached poses', async ({
        page,
      }) => {
        ...
        const filmstripInfo = await page.evaluate(() => {
          ...
          if (!filmstripContainer)
            return { found: false, canvasCount: 0, hasLabels: false };

          const canvases = filmstripContainer.querySelectorAll('canvas');
          const labels =
            filmstripContainer.querySelectorAll('.filmstrip-label');
      ...
      test('filmstrip thumbnails are clickable and seek video', async ({
        page,
      }) => {
        // Seed pose data
        await seedPoseTrackFixture(page, 'swing-sample');
