...
test.describe('Rep Detection Validation', () => {
  test('igor-1h-swing detects 9 reps with correct angles in session recording', async ({ page }) => {
    ...
    await page.waitForFunction(() => {
      ...
      return session?.stateChanges?.some((e: any) => e.type === 'skeleton_processing_complete');
    }, { timeout: 30000 });
    ...
    const completionEvent = session.stateChanges.find((e: any) => e.type === 'skeleton_processing_complete');

    console.log('Session recording results:');
    console.log('- Total rep_detected events:', repEvents.length);
    console.log('- Final rep count from completion:', completionEvent?.details?.finalRepCount);
    console.log('- Frames processed:', completionEvent?.details?.framesProcessed);
    console.log('- Total frames processed:', completionEvent?.details?.totalFramesProcessed);
    ...
    repEvents.forEach((e: any) => {
      ...
      const armToVert = d.angles?.armToVertical != null ? d.angles.armToVertical.toFixed(1) : 'N/A';
      const hip = d.angles?.hip != null ? d.angles.hip.toFixed(0) : 'N/A';
      console.log(`  Rep ${d.repNumber}: frame=${d.frameIndex}, time=${videoTime}s, spine=${spine}, arm=${arm}, armToVert=${armToVert}, hip=${hip}, phase=${d.phase}`);
    });
    ...
    const expectedRepTimes = [4.10, 5.63, 7.17, 8.73, 10.27, 11.87, 13.43, 15.00, 16.57];
  ...
  test('swing-sample (browser-extracted) detects 9 reps', async ({ page }) => {
    ...
    await page.route(/raw\.githubusercontent\.com\/.*form-analyzer-samples.*\.webm/, async (route) => {
    ...
    repEvents.forEach((e: any) => {
      ...
      const spine = d.angles?.spine != null ? d.angles.spine.toFixed(1) : 'N/A';
      const armToVert = d.angles?.armToVertical != null ? d.angles.armToVertical.toFixed(1) : 'N/A';
      console.log(`  Rep ${d.repNumber}: time=${videoTime}s, spine=${spine}, armToVert=${armToVert}, phase=${d.phase}`);
    });
