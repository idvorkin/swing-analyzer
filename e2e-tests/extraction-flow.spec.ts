...
test.describe
  .serial('Extraction Flow: Mock Detector + Real Pipeline', () => {
      ...
      try {
        await page.waitForFunction(
          () => {
            ...
            const repGallery = document.querySelector('.rep-gallery-container');
            const thumbnails =
              repGallery?.querySelectorAll('canvas').length || 0;
            return playBtn && !playBtn.disabled && thumbnails > 0;
          },
        ...
        const sessionData = await page.evaluate(() => {
          const debug = (
            window as unknown as {
              swingDebug?: { getCurrentSession?: () => unknown };
            }
          ).swingDebug;
          if (debug?.getCurrentSession) {
            const session = debug.getCurrentSession() as {
              stateChanges?: unknown[];
              pipelineSnapshots?: unknown[];
            };
            return {
        ...
        console.log(
          'SessionRecorder state on failure:',
 ...
 */
test.describe
  .skip('Igor 1H Swing: Real Sample Video (slow)', () => {
    test.beforeEach(async ({ page }) => {
    ...
    test('full extraction with igor-1h-swing video reproduces user experience', async ({
      ...
      console.log(
        `Igor-1h-swing extraction completed: ${repCount} reps detected`
      );
    ...
    test('skeleton renders after extraction and during playback', async ({
      page,
    }) => {
