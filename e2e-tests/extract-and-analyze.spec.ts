...
test.describe('Fresh Extraction Analysis', () => {
  ...
  test.skip('extracts from remote video and detects 9 reps', async ({ page }) => {
    ...
    await page.waitForFunction(() => {
      ...
      const hasComplete = session?.stateChanges?.some((e: any) =>
        e.type === 'skeleton_processing_complete' || e.type === 'extraction_complete'
      );
      // Also check progress
      const progress = session?.stateChanges?.filter((e: any) => e.type === 'extraction_start');
    ...
    const extractionStart = session.stateChanges.find((e: any) => e.type === 'extraction_start');
    const extractionComplete = session.stateChanges.find((e: any) => e.type === 'extraction_complete');
    const cacheLoad = session.stateChanges.find((e: any) => e.type === 'cache_load');
    const processingComplete = session.stateChanges.find((e: any) => e.type === 'skeleton_processing_complete');

    console.log('\n========== EXTRACTION RESULTS ==========');
    console.log('Cache load:', cacheLoad ? 'YES (from cache)' : 'NO (fresh extraction)');
    ...
    if (processingComplete) {
      console.log('  Frames processed:', processingComplete.details?.framesProcessed);
      console.log('  Final rep count:', processingComplete.details?.finalRepCount);
      console.log('  Processing time:', processingComplete.details?.processingTimeMs, 'ms');
    }
